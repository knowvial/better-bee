// Script to populate word definitions using Free Dictionary API
// Usage: node populate-definitions.js

const fs = require('fs');
const https = require('https');

// Load the current word list directly
            ...
            res.on('end', () => {
                try {
                    if (res.statusCode === 200) {
                        ...
                        const wordEntry = jsonData[0]; // Get first entry
                        
                        // Extract definition (first meaning)
                        let definition = '';
                        ...
                        // If no example found, create a simple one
...
async function populateDefinitions() {
    console.log(`Starting to populate definitions for ${COMPLETE_NSF_WORDS.length} words...`);
    
    const enrichedWords = [];
    let successCount = 0;
    let failCount = 0;
    
    for (let i = 0; i < COMPLETE_NSF_WORDS.length; i++) {
        const word = COMPLETE_NSF_WORDS[i];
        console.log(`[${i + 1}/${COMPLETE_NSF_WORDS.length}] Fetching: ${word}`);
        ...
        if ((i + 1) % 50 === 0) {
            console.log(`\n--- Progress: ${i + 1}/${COMPLETE_NSF_WORDS.length} (${Math.round((i + 1) / COMPLETE_NSF_WORDS.length * 100)}%) ---`);
            console.log(`✅ Success: ${successCount}, ❌ Failed: ${failCount}\n`);
        }
    }
    
    // Generate new complete-words.js file
    const fileContent = `// Complete NSF Junior Spelling Bee 2025 Word List with Definitions
// Generated on ${new Date().toISOString()}
// Success rate: ${successCount}/${COMPLETE_NSF_WORDS.length} words (${Math.round(successCount / COMPLETE_NSF_WORDS.length * 100)}%)

const COMPLETE_NSF_WORDS_WITH_DEFINITIONS = ${JSON.stringify(enrichedWords, null, 4)};
...
if (typeof NSF_WORDS !== 'undefined') {
    ...
    console.log(`\n🎉 Complete! Generated complete-words-with-definitions.js`);
    console.log(`📊 Statistics:`);
    console.log(`   Total words: ${COMPLETE_NSF_WORDS.length}`);
    console.log(`   ✅ Successfully populated: ${successCount} (${Math.round(successCount / COMPLETE_NSF_WORDS.length * 100)}%)`);
    console.log(`   ❌ Failed to populate: ${failCount} (${Math.round(failCount / COMPLETE_NSF_WORDS.length * 100)}%)`);
    console.log(`\nNext steps:`);
    console.log(`1. Review the generated file: complete-words-with-definitions.js`);
    console.log(`2. If satisfied, replace complete-words.js with the new file`);
    console.log(`3. Update index.html to use the new file`);
}
...
module.exports = { populateDefinitions, fetchWordData };